...
struct OnboardingView: View {
    ...
    private var visibleStep: OnboardingStep {
        if step == .name && shouldSkipRequiredNameEntry {
            return .skillLevel
        }
        return step
    }

    private var shouldSkipRequiredNameEntry: Bool {
        authManager.currentUser?.provider == .apple
    }

    private var profileNameForRound: String {
        let typedName = localName.trimmingCharacters(in: .whitespaces)
        if !typedName.isEmpty { return typedName }

        let existingProfileName = appState.profile.name.trimmingCharacters(in: .whitespaces)
        if !existingProfileName.isEmpty { return existingProfileName }

        let authDisplayName = authManager.currentUser?.displayName?.trimmingCharacters(in: .whitespaces)
        if let authDisplayName, !authDisplayName.isEmpty { return authDisplayName }

        return "Golfer"
    }

    private func prepareOnboardingDefaults() {
        if localName.trimmingCharacters(in: .whitespaces).isEmpty {
            localName = profileNameForRound
        }
        if shouldSkipRequiredNameEntry && step == .name {
            step = .skillLevel
        }
    }
    ...
    private func startAsBeginner() async {
        appState.profile.name = profileNameForRound
        appState.profile.handicap = 36
        appState.profile.shot_shape = "straight"
        appState.isBeginnerMode = true
        // Ensure the first-tap mic/scorecard tips show for this brand-new user.
